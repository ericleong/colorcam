	...
	static class LayoutState {
		...
		private View nextFromLimitedList() {
			...
			for (int i = 0; i < size; i++) {
				...
				try {
					Method isRemoved = RecyclerView.ViewHolder.class.getMethod("isRemoved");
					isRemoved.setAccessible(true);
					if (!mIsPreLayout && (boolean) isRemoved.invoke(viewHolder)) {
						continue;
					}
				} catch (NoSuchMethodException e) {
					Log.w(TAG, "Could not find method isRemoved() in RecyclerView.ViewHolder");
				} catch (InvocationTargetException e) {
					Log.w(TAG, "Could not invoke method isRemoved() in RecyclerView.ViewHolder");
				} catch (IllegalAccessException e) {
					Log.w(TAG, "Could not access method isRemoved() in RecyclerView.ViewHolder");
				}
