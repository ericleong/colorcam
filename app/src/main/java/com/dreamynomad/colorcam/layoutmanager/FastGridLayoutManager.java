package com.dreamynomad.colorcam.layoutmanager;
 ...
 */
public class FastGridLayoutManager extends LinearLayoutManager {
	 ...
	 */
	public FastGridLayoutManager(Context context, int spanCount, int orientation,
	...
	public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler,
		...
		if (!(lp instanceof LayoutParams)) {
			try {
				Method method =
						RecyclerView.class.getMethod("onInitializeAccessibilityNodeInfoForItem",
								View.class, AccessibilityNodeInfoCompat.class);
				method.setAccessible(true);
				method.invoke(host, info);
				return;
			} catch (NoSuchMethodException e) {
				Log.w(TAG, "Could not find method onInitializeAccessibilityNodeInfoForItem in " +
						"RecyclerView");
			} catch (InvocationTargetException e) {
				Log.w(TAG, "Could not invoke method onInitializeAccessibilityNodeInfoForItem in " +
						"RecyclerView");
			} catch (IllegalAccessException e) {
				Log.w(TAG, "Could not access method onInitializeAccessibilityNodeInfoForItem in " +
						"RecyclerView");
			}
		}
	...
	void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
		...
		for (int i = 0; i < count; i++) {
			...
			if (mOrientation == VERTICAL &&
					mSizePerSpan * spanSize - lp.leftMargin - lp.rightMargin != lp.width) {
				measureChildWithDecorationsAndMargin(view, spec, getMainDirSpec(lp.height));
				lp.width = view.getMeasuredWidth();
				view.setLayoutParams(lp);
			} else if (mOrientation == HORIZONTAL &&
					mSizePerSpan * spanSize - lp.topMargin - lp.bottomMargin != lp.height) {
	 ...
	 *
	 * @see FastGridLayoutManager#setSpanSizeLookup(SpanSizeLookup)
	 */
